...
Java_org_apache_gluten_vectorized_PlanEvaluatorJniWrapper_nativeCreateKernelWithIterator( // NOLINT
    JNIEnv* env,                       // JVM 环境
    jobject wrapper,                   // 调用此方法的java对象（PlanEvaluatorJniWrapper）
    jbyteArray planArr,                // // Substrait 计划字节数组
    jobjectArray splitInfosArr,        // // 分片信息
    jobjectArray iterArr,              // // 输入迭代器数组
    jint stageId,                      //  // Spark Stage ID
    jint partitionId,                  // 分区信息
    jlong taskId,                      // taskId
    jboolean enableDumping,            // 是否启用调试转储
    jstring spillDir) {                // 溢写目录
  JNI_METHOD_START

  // 运行时上下文初始化, 设置Spark任务信息
  Runtime * ctx = getRuntime(env, wrapper);
